...
let rec lsp (e: E) : string =
    ...
    | EFn (argument, expr, isProc) ->
        sprintf "(%s [%s] %s)" (if isProc then "proc" else "fn") argument
        ^% lsp expr
    ...
    | EObj (fields) ->
        sprintf
            ...
            (String.concat
                ...
                (fields
                 |> Map.toList
                 |> List.map (fun (k, v) -> sprintf ":%s %s" k ^% lsp v)))
    | EWith (expr, fields) ->
        sprintf
            "(with %s %s)"
            (lsp expr)
...
let ``Parse concat`` () =
    let strings =
        tokenize "fn ss -> { s1 with raw: ffi.concat { s1: ss.s1.raw, s2: ss.s2.raw } }"
...
let ``Parse concat2`` () =
    ...
    Assert.Equal(
        "(fn [ss] (let [s1 (:s1 ss)] (let [s2 (:s2 ss)] (let [s1raw (:raw s1)] (let [s2raw (:raw s2)] (let [concatinput { :s1 s1raw, :s2 s2raw }] (let [concat (:concat ffi)] (let [sout (concat concatinput)] (let [out (with s1 :raw sout)] out)))))))))",
        lisp
    )
