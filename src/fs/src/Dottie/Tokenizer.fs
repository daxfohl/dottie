...
let complete(currentToken: char IList, tokens: PageToken Stack, charNumber: int, lineNumber: int, cNext: char option, cAfter: char option, state: State ref) =
...
let tokenizeLine (line: string, lineNumber: int): PageToken list =
  ...
  for charId = 0 to line.Length - 1 do
    ...
    | InNumber true ->
      if Char.IsNumber(c) then currentToken.Add(c)
      else completeAndLoad()
